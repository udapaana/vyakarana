#!/usr/bin/env python3
"""
Extract rules 51-100 from phase2_cleaned pages to phase3_rules/core/raw/
"""
...
def extract_rule_number(rule_str):
    ...
    match = re.search(r'§\s*(\d+)', rule_str)
    if match:
        return int(match.group(1))
    return None

def collect_rule_pages(start_rule=51, end_rule=100):
    """Scan all pages and collect which pages contain which rules"""
    rule_pages = defaultdict(list)  # {51: [page_040.md, page_041.md], ...}

    print(f"📖 Scanning for rules {start_rule}-{end_rule}...")
    ...
    for page_file in sorted(CLEANED_DIR.glob("page_*.md")):
        ...
        # Check rules_starting
        for rule in rules_starting:
            rule_num = extract_rule_number(str(rule))
...
def extract_rule_content(rule_num, pages_info):
    ...
    for page_info in pages_info:
        ...
        # Split into frontmatter, body, and footnotes
        ...
        if page_info['type'] == 'start':
            ...
            # Continuing page - take content until next §
            next_rule = re.search(r'\n§\s*\d+\.', body)
            if next_rule:
...
def main():
    print("🚀 Extracting rules 51-100...\n")

    # Collect which pages contain which rules
    rule_pages = collect_rule_pages(51, 100)

    if not rule_pages:
        print("❌ No rules found in range 51-100")
        return

    print(f"✓ Found {len(rule_pages)} rules\n")

    # Extract and create each rule file
    extracted = 0
    for rule_num in sorted(rule_pages.keys()):
        try:
            ...
            print(f"✓ Rule {rule_num:3d}: pages {rule_data['page_start']}-{rule_data['page_end']} → {output_file.name}")
            extracted += 1
        except Exception as e:
            print(f"❌ Rule {rule_num}: {e}")

    print(f"\n✅ Complete: extracted {extracted} rules")
