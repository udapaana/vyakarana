---
rule: § 949
page: 526
source_pages:
  dli: [526]
  official_1931: [540]
chapter: Syntax
section: moods
subsections: [imperative]
topics: [imperative-mood, frequency, repetition]

hierarchy:
  chapter: Syntax
  section: moods

word_index: []

panini_refs:
  - "III. 4. 1-5"

cross_refs:
  - "§ 948"
  - "§ 950"

confidence: high

image: /images/512.png
---

## § 949. Repeated Imperative for Frequency

There is a peculiar use of the Imperative which ought to be noticed. The Imperative second person singular is repeated when frequency of an act is indicated and the whole used with the root in any tense:

**Examples:**

- @deva[याहि याहि हति गति] (Sid. Kau.) he goes every now and then
- @deva[पकाहि पकाहि पक्षां पकाहि] (Sid Kau) he cooks
- @deva[अध्यापय अध्यायते] he studies steadily

The Imperative second person is also used when several acts are described as done by the same person:

- @deva[शकुच्छ फिरु, पानः सुदचंयुहारति] (Sid. Kau.) he takes his meal, now eating fruit, now eating fried rice
- Similarly: @deva[अखनं वदय दविकंयवरयते] (Sid. Kau.)

---

**Pāṇini Reference:** @deva[क्रियासयनेयुरो लोट् छन्दसो छन्दोतो हित्यदो वा च तस्यानो ।] (Pāṇ. III. 4. 1-5)
